...
export function CalloutPanel({
  ...
  return (
    <aside
      className={`panel${dragging ? " callouts-dragging" : ""}`}
      ...
      {selectedCallout ? (
        <div className="selected">
          <label>
            Name
            <input
              value={selectedCallout.name}
              onChange={(e) => onRename(selectedCallout.id, e.target.value)}
            />
          </label>
          <div className="selected-order">
            <button
              ...
              onClick={() => onNudge(selectedCallout.id, -1)}
            >
              ↑
            </button>
            <button
              type="button"
              disabled={selectedIndex < 0 || selectedIndex >= layout.callouts.length - 1}
              onClick={() => onNudge(selectedCallout.id, 1)}
            >
              ↓
            </button>
          ...
          <p className="muted">
            {selectedCallout.floor === "lower" ? "Lower" : "Upper"} ·{" "}
            {selectedCallout.polygon.length} vertices · {selectedCallout.id}
          </p>
          <button type="button" className="danger" onClick={() => onDelete(selected)}>
            Delete
          </button>
        </div>
      ) : null}
