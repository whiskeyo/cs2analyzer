...
import { strokesToNote, strokesToRoundNotes } from "./migrate";
...
describe("strokesToRoundNotes", () => {
  it("buckets by round", () => {
    ...
    expect(rows[0]?.note.groups[0]?.id).toBe("g1");
    expect(rows[1]?.note.drawings).toHaveLength(1);
  });

  it("keeps a grouped moment window on the layer", () => {
    const note = strokesToNote([
      pen({ group: "g1", start_tick: 10, end_tick: 20 }),
      pen({ group: "g1", start_tick: 15, end_tick: 40 }),
    ]);
    expect(note.groups[0]).toMatchObject({ start_tick: 10, end_tick: 40 });
