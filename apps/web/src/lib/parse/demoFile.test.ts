...
describe("demoFileNameIssue", () => {
  ...
  it("rejects gzip names and MIME without reading bytes", () => {
    expect(demoFileNameIssue(fileWithSize("match.dem.gz", 12))).toContain("unsupported gzip");
    expect(demoFileNameIssue(fileWithSize("match.gz", 12))).toContain("unsupported gzip");
    expect(
      demoFileNameIssue(new File(["xxxx"], "match.dem", { type: "application/gzip" })),
    ).toContain("unsupported gzip");
  });

  it("rejects a non-.dem extension", () => {
    expect(demoFileNameIssue(fileWithSize("clip.mp4", 40))).toContain("not a .dem file");
    expect(demoFileNameIssue(fileWithSize("notes.txt", 40))).toContain("not a .dem file");
  });

  it("rejects a POV-looking filename", () => {
    expect(demoFileNameIssue(fileWithSize("player_pov.dem", 40))).toContain("POV demo");
    expect(demoFileNameIssue(fileWithSize("POV.dem", 40))).toContain("POV demo");
...
describe("sniffDemoMagic", () => {
  it("detects gzip, Source 1, and non-CS2 payloads", () => {
    expect(sniffDemoMagic(new Uint8Array([0x1f, 0x8b, 0x08]), "a.dem")).toContain(
      "unsupported gzip",
    );
    expect(sniffDemoMagic(new TextEncoder().encode(SOURCE1_DEMO_MAGIC), "old.dem")).toContain(
      "Source 1",
    );
    expect(sniffDemoMagic(new TextEncoder().encode("notademo"), "fake.dem")).toContain(
      "not a Counter-Strike 2 demo",
    );
    expect(sniffDemoMagic(new TextEncoder().encode(CS2_DEMO_MAGIC), "gotv.dem")).toBeNull();
  });
});
...
describe("inspectDemoFile", () => {
  it("accepts a CS2 magic stub and rejects a gzip payload named .dem", async () => {
    ...
    const gzip = new File([new Uint8Array([0x1f, 0x8b, 0x08, 0x00])], "gotv.dem");
...
describe("partitionDemoFiles", () => {
  it("keeps valid demos and records issues", async () => {
    ...
    const { ok, issues } = await partitionDemoFiles([empty, good, new File(["nope"], "clip.mp4")]);
...
describe("formatParseError", () => {
  it("maps Source 2 / truncated / corrupt parser failures", () => {
    expect(formatParseError("Supports only Source 2 replays", "bad.dem")).toContain(
      "not a Counter-Strike 2 demo",
    );
    expect(formatParseError("unexpected eof in packet", "cut.dem")).toContain("truncated");
    expect(formatParseError("failed to parse demo: junk", "x.dem")).toContain("corrupt");
  });
  ...
  it("leaves Wasm fetch and decode errors alone", () => {
    ...
    expect(formatParseError("header: expected object")).toBe("header: expected object");
  });
});
