...
describe("PlaybookCanvas", () => {
  ...
  it("repaints when radarGray changes", () => {
    ...
    expect(paintPlaybookBoard).toHaveBeenCalledTimes(1);
    expect(vi.mocked(paintPlaybookBoard).mock.calls.at(-1)?.[15]).toBe(1);
    rerender(
      <PlaybookCanvas cal={UNIT_CALIBRATION} floorMode="auto" note={emptyNote()} radarGray={0} />,
    );
    act(() => {
      rafCb?.(1);
    });
